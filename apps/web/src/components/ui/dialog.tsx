...
export function Dialog({ open, onOpenChange, children, contentClassName, ariaLabel }: DialogProps) {
  ...
  if (!open) return null;
  return <div className="fixed inset-0 z-[100] grid place-items-center p-4" role="presentation"><button aria-label="Close dialog" className="absolute inset-0 cursor-default bg-slate-950/30" onClick={() => onOpenChange(false)} /><div role="dialog" aria-modal="true" aria-label={ariaLabel} className={cn('relative w-full max-w-md glass-surface rounded-2xl p-6', contentClassName)}>{children}</div></div>;
}
