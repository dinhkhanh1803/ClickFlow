...
export function ContextSidebar({ modulePath, preview = false, onCollapse }: ContextSidebarProps) {
  ...
  useEffect(() => {
    ...
    setActiveSpaceId(savedSpaces[0]?.id ?? '');
    setParentSpaceId(savedSpaces[0]?.id ?? '');
    setHydrated(true);
  }, []);
  ...
  const beginListInProject = (spaceId: string, folderId: string) => {
    setCreateKind('list');
    setNewName('');
    setParentSpaceId(spaceId);
  ...
  const runProjectAction = (action: string, spaceId: string, folderId: string) => {
    ...
    if (action === 'rename') {
      ...
      beginListInProject(spaceId, folderId);
    } else {
      notifyToastPreview();
    }
  ...
  return <aside onClick={(event) => { if (!(event.target as HTMLElement).closest('[data-options-menu], [data-options-trigger]')) { setProjectMenuId(null); setSpaceMenuId(null); setListMenuId(null); setSpaceCreateMenuId(null); } }} aria-label={`${context.title} ${preview ? 'preview' : 'navigation'}`} className={`glass-shell hidden h-screen w-72 shrink-0 flex-col border-r border-white/30 p-3 md:flex ${preview ? 'absolute left-0 top-0 z-20 rounded-r-2xl border border-white/45 shadow-2xl shadow-slate-950/20' : ''}`}>
    ...
    {isSpaces && <><div className="my-4 border-t border-white/40 dark:border-slate-700/60" /><div className="flex items-center justify-between px-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{context.section}</p><button type="button" aria-label="Create a new Space" onClick={() => beginCreate('space')} className="rounded p-1 text-slate-500 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-slate-800"><Plus size={15} /></button></div><nav className="mt-2 space-y-1">{spaces.map((space) => <div key={space.id}><div className="relative flex items-center"><button type="button" aria-label={`${expandedSpaceIds.has(space.id) ? 'Collapse' : 'Expand'} ${space.name}`} onClick={() => toggleTree(space.id, setExpandedSpaceIds)} className="ml-1 rounded p-0.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-white">{expandedSpaceIds.has(space.id) ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</button><Link href={`/projects?space=${space.id}`} onClick={() => setActiveSpaceId(space.id)} aria-current={space.id === activeSpaceId ? 'page' : undefined} className={`flex min-w-0 flex-1 items-center gap-3 rounded-lg px-3 py-2 text-sm transition ${space.id === activeSpaceId ? 'bg-slate-900/10 font-semibold text-slate-900 dark:bg-white/10 dark:text-white' : 'text-slate-600 hover:bg-white/55 dark:text-slate-300 dark:hover:bg-slate-800/70'}`}><span className={`h-5 w-5 rounded-md ${space.tone}`} /><span className="truncate">{space.name}</span>{space.private && <span className="ml-auto text-xs font-normal text-slate-400">Private</span>}</Link><button type="button" data-options-trigger aria-label={`Create in ${space.name}`} onClick={() => { setSpaceCreateMenuId((current) => current === space.id ? null : space.id); setSpaceMenuId(null); setProjectMenuId(null); setListMenuId(null); }} className="rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-slate-800"><Plus size={15} /></button>{spaceCreateMenuId === space.id && <div role="menu" data-options-menu className="absolute right-2 top-8 z-40 w-48 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-950/15 dark:border-slate-700 dark:bg-slate-900"><p className="px-3 py-1 text-xs font-semibold text-slate-500">Create in Space</p><button type="button" role="menuitem" onClick={() => beginCreateInSpace('folder', space.id)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800"><Folder size={15} />Folder</button><button type="button" role="menuitem" onClick={() => beginCreateInSpace('list', space.id)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800"><ListChecks size={15} />List</button></div>}<button type="button" data-options-trigger aria-label={`More options for ${space.name}`} onClick={() => { setSpaceMenuId((current) => current === space.id ? null : space.id); setProjectMenuId(null); setListMenuId(null); setSpaceCreateMenuId(null); }} className="mr-2 rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-slate-800"><MoreHorizontal size={15} /></button>{spaceMenuId === space.id && <div role="menu" data-options-menu className="absolute left-8 top-8 z-40 w-48 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-950/15 dark:border-slate-700 dark:bg-slate-900"><button type="button" role="menuitem" onClick={() => runSpaceAction('rename', space.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Rename</button><button type="button" role="menuitem" onClick={() => runSpaceAction('copy-link', space.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Copy link</button><button type="button" role="menuitem" onClick={() => runSpaceAction('duplicate', space.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Duplicate</button><div className="my-1 border-t border-slate-100 dark:border-slate-800" /><button type="button" role="menuitem" onClick={() => runSpaceAction('delete', space.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30">Delete</button></div>}</div>{space.id === activeSpaceId && expandedSpaceIds.has(space.id) && space.items.filter((item) => !item.parentId).map((item) => { const Icon = itemIcon(item.kind); const childLists = space.items.filter((child) => child.parentId === item.id); return item.kind === 'folder' ? <div key={item.id}><div className="relative flex items-center gap-1"><button type="button" aria-label={`${expandedFolderIds.has(item.id) ? 'Collapse' : 'Expand'} ${item.name}`} onClick={() => toggleTree(item.id, setExpandedFolderIds)} className="ml-7 rounded p-0.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-white">{expandedFolderIds.has(item.id) ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</button><button type="button" className="flex min-w-0 flex-1 items-center gap-2 rounded-lg py-1.5 pl-1 pr-1 text-left text-sm text-slate-500 hover:bg-white/55 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/70 dark:hover:text-white"><Icon size={15} />{item.name}</button><div className="mr-2 flex items-center"><button type="button" aria-label={`Create list in ${item.name}`} onClick={() => beginListInProject(space.id, item.id)} className="rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-slate-800"><Plus size={15} /></button><button type="button" data-options-trigger aria-label={`More options for ${item.name}`} onClick={() => { setProjectMenuId((current) => current === item.id ? null : item.id); setSpaceMenuId(null); setListMenuId(null); setSpaceCreateMenuId(null); }} className="rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-slate-800"><MoreHorizontal size={15} /></button></div>{projectMenuId === item.id && <div role="menu" data-options-menu className="absolute left-10 top-8 z-40 w-56 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-950/15 dark:border-slate-700 dark:bg-slate-900"><button type="button" role="menuitem" onClick={() => runProjectAction('favorite', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Favorite</button><button type="button" role="menuitem" onClick={() => runProjectAction('rename', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Rename</button><button type="button" role="menuitem" onClick={() => runProjectAction('copy-link', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Copy link</button><div className="my-1 border-t border-slate-100 dark:border-slate-800" /><button type="button" role="menuitem" onClick={() => runProjectAction('create-list', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Create new</button><button type="button" role="menuitem" onClick={() => runProjectAction('folder', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Folder</button><button type="button" role="menuitem" onClick={() => runProjectAction('automations', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Automations</button><button type="button" role="menuitem" onClick={() => runProjectAction('custom-fields', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Custom Fields</button><button type="button" role="menuitem" onClick={() => runProjectAction('task-statuses', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Task statuses</button><button type="button" role="menuitem" onClick={() => runProjectAction('more', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">More</button><div className="my-1 border-t border-slate-100 dark:border-slate-800" /><button type="button" role="menuitem" onClick={() => runProjectAction('imports', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Imports</button><button type="button" role="menuitem" onClick={() => runProjectAction('templates', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Templates</button><div className="my-1 border-t border-slate-100 dark:border-slate-800" /><button type="button" role="menuitem" onClick={() => runProjectAction('move', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Move</button><button type="button" role="menuitem" onClick={() => runProjectAction('duplicate', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Duplicate</button><button type="button" role="menuitem" onClick={() => runProjectAction('archive', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Archive</button><button type="button" role="menuitem" onClick={() => runProjectAction('delete', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30">Delete</button></div>}</div>{expandedFolderIds.has(item.id) && childLists.map((child) => { const ChildIcon = itemIcon(child.kind); return <div key={child.id} className="relative flex items-center"><button type="button" className="flex min-w-0 flex-1 items-center gap-2 rounded-lg py-1.5 pl-[3.8rem] pr-1 text-left text-sm text-slate-500 hover:bg-white/55 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/70 dark:hover:text-white"><ChildIcon size={14} />{child.name}</button><button type="button" data-options-trigger aria-label={`More options for ${child.name}`} onClick={() => { setListMenuId((current) => current === child.id ? null : child.id); setProjectMenuId(null); setSpaceMenuId(null); setSpaceCreateMenuId(null); }} className="mr-2 rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-slate-800"><MoreHorizontal size={15} /></button>{listMenuId === child.id && <div role="menu" data-options-menu className="absolute left-12 top-8 z-40 w-44 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-950/15 dark:border-slate-700 dark:bg-slate-900"><button type="button" role="menuitem" onClick={() => runListAction('rename', space.id, child.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Rename</button><button type="button" role="menuitem" onClick={() => runListAction('copy-link', space.id, child.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Copy link</button><button type="button" role="menuitem" onClick={() => runListAction('duplicate', space.id, child.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Duplicate</button><div className="my-1 border-t border-slate-100 dark:border-slate-800" /><button type="button" role="menuitem" onClick={() => runListAction('delete', space.id, child.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30">Delete</button></div>}</div>; })}</div> : item.kind === 'list' ? <div key={item.id} className="relative flex items-center"><button type="button" className="flex min-w-0 flex-1 items-center gap-2 rounded-lg py-1.5 pl-11 pr-1 text-left text-sm text-slate-500 hover:bg-white/55 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/70 dark:hover:text-white"><Icon size={15} />{item.name}</button><button type="button" data-options-trigger aria-label={`More options for ${item.name}`} onClick={() => { setListMenuId((current) => current === item.id ? null : item.id); setProjectMenuId(null); setSpaceMenuId(null); setSpaceCreateMenuId(null); }} className="mr-2 rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-slate-800"><MoreHorizontal size={15} /></button>{listMenuId === item.id && <div role="menu" data-options-menu className="absolute left-12 top-8 z-40 w-44 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-950/15 dark:border-slate-700 dark:bg-slate-900"><button type="button" role="menuitem" onClick={() => runListAction('rename', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Rename</button><button type="button" role="menuitem" onClick={() => runListAction('copy-link', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Copy link</button><button type="button" role="menuitem" onClick={() => runListAction('duplicate', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800">Duplicate</button><div className="my-1 border-t border-slate-100 dark:border-slate-800" /><button type="button" role="menuitem" onClick={() => runListAction('delete', space.id, item.id)} className="w-full rounded-lg px-3 py-2 text-left text-sm text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30">Delete</button></div>}</div> : <button key={item.id} type="button" className="flex w-full items-center gap-2 rounded-lg py-1.5 pl-11 pr-3 text-left text-sm text-slate-500 hover:bg-white/55 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/70 dark:hover:text-white"><Icon size={15} />{item.name}</button>; })}</div>)}</nav></>}
