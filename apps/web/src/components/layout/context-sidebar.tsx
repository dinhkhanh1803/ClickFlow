...
import { CreationDialog, type CreationDialogKind } from '@/features/workspace/components/creation-dialog';
...
import { useArchiveProjectMutation, useArchiveWorkspaceMutation, useArchiveSectionMutation, useCreateProjectMutation, useCreateRootSectionMutation, useCreateSectionMutation, useCreateWorkspaceMutation, useUpdateWorkspaceMutation, useUpdateProjectMutation, useUpdateSectionMutation, useWorkspaceNavigationQuery } from '@/features/workspace/data/workspace-queries';
...
export function ContextSidebar({ modulePath, preview = false, onCollapse }: ContextSidebarProps) {
  ...
  const submitSpaceSettings = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (navigationQuery.usesApi) {
      try {
        await updateWorkspaceMutation.mutateAsync({ workspaceId: settingsSpaceId, input: { name, private: settingsPrivate, tone: `appearance:${settingsIcon}|${settingsTone}` } });
        setSettingsSpaceId(null);
        toast.success('Space settings updated.');
      } catch { toast.error('Unable to update Space settings.'); }
      return;
    }
    const nextSpaces = spaces.map((space) => space.id === settingsSpaceId ? { ...space, name, private: settingsPrivate, icon: settingsIcon, tone: settingsTone } : space);
  ...
  const submitCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!navigationQuery.usesApi) {
      if (createKind === 'space') {
        ...
        setSpaces((current) => [...current, { id, name, tone: nextSpaceTone(current.length), private: newSpacePrivate, items: [] }]);
        setActiveSpaceId(id);
      } else {
    ...
    if (createKind === 'space') {
      try {
        const workspace = await createWorkspaceMutation.mutateAsync({ name, private: newSpacePrivate });
...
<SpaceSettingsDialog open={settingsSpaceId !== null} name={settingsName} isPrivate={settingsPrivate} icon={settingsIcon} tone={settingsTone} pending={updateWorkspaceMutation.isPending} onOpenChange={(open) => { if (!open) setSettingsSpaceId(null); }} onNameChange={setSettingsName} onPrivateChange={setSettingsPrivate} onIconChange={setSettingsIcon} onToneChange={setSettingsTone} onSubmit={submitSpaceSettings} />
    <CreationDialog kind={createKind as CreationDialogKind} open={createOpen} name={newName} description={newDescription} isPrivate={newSpacePrivate} invitees={newSpaceInvitees} pending={creationPending} parentLabel={creationParent} onNameChange={setNewName} onDescriptionChange={setNewDescription} onPrivateChange={setNewSpacePrivate} onInviteesChange={setNewSpaceInvitees} onOpenChange={setCreateOpen} onSubmit={submitCreate} />
  </aside>;
}