...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="flex min-h-screen"><AppSidebar onPreviewModuleChange={setPreviewModule} onModuleSelect={() => setContextPanelCollapsed(false)} /><div className={`relative hidden h-screen shrink-0 md:block ${contextPanelCollapsed ? 'w-0' : 'w-72'}`}>{!contextPanelCollapsed && <ContextSidebar onCollapse={() => setContextPanelCollapsed(true)} />}{previewModule && <ContextSidebar modulePath={previewModule} preview />}</div><main className="min-w-0 flex-1"><AppHeader />{children}</main></div>;
}