...
const workspace = { id: 'workspace-1', name: 'Product', description: 'Product planning', tone: null, private: false, publicAccess: 'EDIT', timezone: 'UTC', locale: 'en', role: 'OWNER', createdBy: { id: 'user-1', displayName: 'Owner User', avatarUrl: null }, createdAt: timestamp, updatedAt: timestamp } satisfies WorkspaceResponse;
...
describe('mapWorkspaceTree', () => {
  it('maps Workspace, Project and Section to the canonical local navigation tree', () => {
    expect(mapWorkspaceTree([workspace], [project], [section])).toEqual([{
      id: 'workspace-1', name: 'Product', tone: 'bg-indigo-500', description: 'Product planning', private: false, publicAccess: 'EDIT', createdBy: { id: 'user-1', displayName: 'Owner User', avatarUrl: null }, role: 'OWNER',
      items: [
