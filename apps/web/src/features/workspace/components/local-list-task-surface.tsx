...
type LocalListTaskSurfaceProps = {
  ...
  onOpenAttachment?: (attachment: LocalTaskAttachment) => void;
};
...
function TaskAttachmentSection({ attachments, onAdd, onRemove, onPreview, onOpen }: { attachments: LocalTaskAttachment[]; onAdd: (files: File[]) => void; onRemove: (attachmentId: string) => void; onPreview: (attachmentId: string) => void; onOpen?: (attachment: LocalTaskAttachment) => void }) {
  ...
  return <section className="mt-7 border-t border-slate-200 pt-6 dark:border-slate-800" aria-labelledby="task-attachments-title">
    ...
    {attachments.length > 0 && <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{attachments.map((attachment) => {
      ...
      const canPreviewLocally = Boolean(attachment.dataUrl) && (kind === 'image' || kind === 'video');
      const open = () => canPreviewLocally ? onPreview(attachment.id) : onOpen?.(attachment);
      return <article key={attachment.id} className="group relative overflow-hidden rounded-xl border border-slate-200 bg-slate-50 p-2 dark:border-slate-800 dark:bg-slate-900"><button type="button" aria-label={`${canPreviewLocally ? 'Preview' : 'Open'} attachment ${attachment.name}`} onClick={open} className="block h-24 w-full overflow-hidden rounded-lg bg-white text-left outline-none ring-indigo-500 focus:ring-2 dark:bg-slate-950">{kind === 'image' && attachment.dataUrl ? <img src={attachment.dataUrl} alt="" className="h-full w-full object-cover transition duration-200 group-hover:scale-[1.03]" /> : kind === 'video' && attachment.dataUrl ? <video src={attachment.dataUrl} muted className="h-full w-full object-cover" /> : <span className="flex h-full items-center justify-center"><FileText aria-hidden="true" size={26} className="text-indigo-500" /></span>}</button><p className="mt-2 truncate text-xs font-medium" title={attachment.name}>{attachment.name}</p><p className="mt-0.5 text-[11px] text-slate-400">{kind.toUpperCase()} {String.fromCharCode(0x00B7)} {Math.max(1, Math.ceil(attachment.size / 1024))} KB</p><button type="button" aria-label={`Remove attachment ${attachment.name}`} onClick={() => onRemove(attachment.id)} className="absolute right-2 top-2 grid h-7 w-7 place-items-center rounded-md bg-slate-950/70 text-white opacity-0 transition hover:bg-rose-600 group-hover:opacity-100 focus:opacity-100"><X aria-hidden="true" size={14} /></button></article>;
    })}</div>}
  </section>;
}
