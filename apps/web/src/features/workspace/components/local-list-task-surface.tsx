...
export function LocalListTaskSurface({ view, tasks, statusGroups, statusOverrides, assignees = [], onCreateStatus, onUpdateStatus, onDeleteStatus, onCreateTask, onUpdateTask, onDeleteTasks }: LocalListTaskSurfaceProps) {
...
export function LocalTaskDetailModal({ task, statusOptions, assignees = [], onClose, onUpdate }: { task: LocalListTask; statusOptions: LocalTaskStatusConfig[]; assignees?: AssigneeOption[]; onClose: () => void; onUpdate: (patch: Partial<Omit<LocalListTask, 'id' | 'createdAt'>>) => void }) {
  ...
  const postComment = ({ body, attachments: commentAttachments, links }: CommentDraft) => {
    ...
    onUpdate({ comments: [...comments, { id: localId('comment'), body: trimmedBody, attachments: commentAttachments, links, authorName: MOCK_COMMENT_AUTHOR, createdAt: new Date().toISOString() }] });
  };
  const addAttachment = (file: File) => {
    const reader = new FileReader();
    reader.onload = () => onUpdate({ attachments: [...attachments, { id: localId('attachment'), name: file.name, mimeType: file.type || 'application/octet-stream', size: file.size, dataUrl: typeof reader.result === 'string' ? reader.result : '', createdAt: new Date().toISOString() }] });
...
function TaskAttachmentSection({ attachments, onAdd, onRemove, onPreview }: { attachments: LocalTaskAttachment[]; onAdd: (file: File) => void; onRemove: (attachmentId: string) => void; onPreview: (attachmentId: string) => void }) {
  ...
  return <section className="mt-7 border-t border-slate-200 pt-6 dark:border-slate-800" aria-labelledby="task-attachments-title">
    ...
    <input id="task-attachment-input" aria-label="Add attachment" type="file" className="sr-only" onChange={(event) => { addFirstFile(event.currentTarget.files); event.currentTarget.value = ''; }} />
    <label htmlFor="task-attachment-input" onDragOver={(event) => { event.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={(event) => { event.preventDefault(); setIsDragging(false); addFirstFile(event.dataTransfer.files); }} className={`mt-3 flex cursor-pointer items-center justify-center rounded-xl border border-dashed px-4 py-4 text-sm transition ${isDragging ? 'border-indigo-500 bg-indigo-50 text-indigo-700 dark:bg-indigo-950/30 dark:text-indigo-200' : 'border-slate-300 text-slate-500 hover:border-indigo-400 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-900'}`}>Drop files here or <span className="ml-1 font-semibold text-indigo-600">browse</span></label>
    {attachments.length > 0 && <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{attachments.map((attachment) => { const isImage = attachment.mimeType.startsWith('image/') && Boolean(attachment.dataUrl); return <article key={attachment.id} className="group relative overflow-hidden rounded-xl border border-slate-200 bg-slate-50 p-2 dark:border-slate-800 dark:bg-slate-900">{isImage ? <button type="button" aria-label={`Preview attachment ${attachment.name}`} onClick={() => onPreview(attachment.id)} className="block h-24 w-full overflow-hidden rounded-lg bg-white text-left outline-none ring-indigo-500 focus:ring-2 dark:bg-slate-950"><img src={attachment.dataUrl} alt="" className="h-full w-full object-cover transition duration-200 group-hover:scale-[1.03]" /></button> : <div className="flex h-24 items-center justify-center rounded-lg bg-white dark:bg-slate-950"><Paperclip aria-hidden="true" size={24} className="text-indigo-500" /></div>}<p className="mt-2 truncate text-xs font-medium" title={attachment.name}>{attachment.name}</p><p className="mt-0.5 text-[11px] text-slate-400">{Math.max(1, Math.ceil(attachment.size / 1024))} KB</p><button type="button" aria-label={`Remove attachment ${attachment.name}`} onClick={() => onRemove(attachment.id)} className="absolute right-2 top-2 grid h-7 w-7 place-items-center rounded-md bg-slate-950/70 text-white opacity-0 transition hover:bg-rose-600 group-hover:opacity-100 focus:opacity-100"><X aria-hidden="true" size={14} /></button></article>; })}</div>}
  </section>;
}

function AttachmentPreview({ attachment, onClose }: { attachment: LocalTaskAttachment; onClose: () => void }) {
  return <div className="fixed inset-0 z-[120] grid place-items-center p-4" role="presentation"><button type="button" aria-label="Close attachment preview" onClick={onClose} className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm" /><section role="dialog" aria-modal="true" aria-label="Attachment preview" className="relative flex max-h-full w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-white/15 bg-slate-950 shadow-2xl"><header className="flex items-center justify-between gap-3 border-b border-white/10 px-5 py-3 text-sm text-white"><span className="truncate font-semibold">{attachment.name}</span><button type="button" aria-label="Close attachment preview" onClick={onClose} className="rounded-md p-2 text-slate-300 hover:bg-white/10 hover:text-white"><X size={18} /></button></header><div className="grid min-h-0 flex-1 place-items-center bg-black/40 p-6"><img src={attachment.dataUrl} alt={attachment.name} className="max-h-[78vh] max-w-full object-contain" /></div></section></div>;
}
function CommentActivityItem({ item }: { item: LocalTaskComment }) {
  const attachments = item.attachments ?? [];
  const links = item.links ?? [];
  const authorName = item.authorName ?? MOCK_COMMENT_AUTHOR;
  const initials = authorName.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase();
  return <div className="rounded-lg border border-indigo-100 bg-indigo-50/60 p-3 dark:border-indigo-950 dark:bg-indigo-950/30"><div className="flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2"><span aria-hidden="true" className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-indigo-500 text-[9px] font-bold text-white">{initials}</span><span className="truncate text-xs font-semibold">{authorName}</span></div><span className="shrink-0 text-[11px] text-slate-400">{new Date(item.createdAt).toLocaleString()}</span></div>{item.body && <p className="mt-2">{item.body}</p>}{attachments.length > 0 && <div className="mt-2 space-y-1.5">{attachments.map((attachment) => <div key={attachment.id} className="flex items-center gap-2 rounded-md bg-white/80 px-2 py-1.5 text-xs dark:bg-slate-950/70">{attachment.mimeType.startsWith('image/') && attachment.dataUrl ? <img src={attachment.dataUrl} alt="" className="h-7 w-7 rounded object-cover" /> : <FileText aria-hidden="true" size={15} className="text-indigo-500" />}<span className="truncate">{attachment.name}</span></div>)}</div>}{links.length > 0 && <div className="mt-2 space-y-1">{links.map((link) => <a key={link} href={link} target="_blank" rel="noreferrer" className="block truncate text-xs font-medium text-indigo-600 hover:underline dark:text-indigo-300">{link}</a>)}</div>}</div>;
}
function CommentComposer({ onSend }: { onSend: (draft: CommentDraft) => void }) {
  ...
  const addFiles = (files: FileList | null) => Array.from(files ?? []).forEach((file) => {
    const reader = new FileReader();
    reader.onload = () => setAttachments((current) => [...current, { id: localId('comment-file'), name: file.name, mimeType: file.type || 'application/octet-stream', size: file.size, dataUrl: typeof reader.result === 'string' ? reader.result : '', createdAt: new Date().toISOString() }]);
    reader.readAsDataURL(file);
  });
  const attachLink = () => {
...
export function StatusPicker({ value, statusGroupId, options, onChange, open, onOpenChange, triggerLabel }: { value: LocalTaskStatus; statusGroupId?: string; options: StatusConfig[]; onChange: (patch: Pick<LocalListTask, 'status' | 'statusGroupId'>) => void; open: boolean; onOpenChange: (open: boolean) => void; triggerLabel?: string }) {
  ...
  const filteredPeople = people.filter((person) => {
    const isCurrentUser = currentUserId !== null && person.userId === currentUserId;
    const displayLabel = isCurrentUser ? 'Me' : person.displayName;
    return !normalizedQuery || displayLabel.toLowerCase().includes(normalizedQuery) || person.displayName.toLowerCase().includes(normalizedQuery) || (person.email?.toLowerCase().includes(normalizedQuery) ?? false);
  });
  const inviteEmail = query.trim().includes('@') ? query.trim() : '';
  const selectedIsCurrentUser = currentUserId !== null && assigneeId === currentUserId;
  const triggerLabel = selectedIsCurrentUser ? 'Me' : value || 'Empty';
  return <div ref={popoverRef} className="relative w-full max-w-xs"><button type="button" aria-label="Edit assignee" onClick={() => { onOpenChange(!open); setQuery(''); }} className="w-full rounded-md px-2 py-1 text-left text-sm text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800">{triggerLabel}</button>{open && <div role="listbox" aria-label="Task assignees" className="absolute left-0 top-[calc(100%+0.5rem)] z-20 w-72 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900"><div className="p-3"><div className="flex items-center gap-2 rounded-md border border-indigo-400 px-2 py-1.5"><Search aria-hidden="true" size={14} className="text-slate-400" /><input autoFocus aria-label="Search assignees" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search or enter email..." className="min-w-0 flex-1 bg-transparent text-sm outline-none" /></div></div><div className="border-t border-slate-100 px-3 py-2 dark:border-slate-800"><p className="text-xs font-medium text-slate-500">People</p><button type="button" role="option" aria-selected={!assigneeId && !value} onClick={() => { onChange({ assignee: '', assigneeId: null }); onOpenChange(false); }} className="mt-2 flex w-full items-center gap-2 rounded-md px-1 py-1.5 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800"><span aria-hidden="true" className="grid h-6 w-6 place-items-center rounded-full bg-slate-300 text-[10px] font-bold text-white dark:bg-slate-700">--</span><span className="flex-1">Unassigned</span>{!assigneeId && !value && <Check aria-hidden="true" size={14} />}</button>{filteredPeople.map((person) => { const isCurrentUser = currentUserId !== null && person.userId === currentUserId; const displayLabel = isCurrentUser ? 'Me' : person.displayName; const active = assigneeId === person.userId || (!assigneeId && value === person.displayName) || (!assigneeId && isCurrentUser && value === 'Me'); const avatarStyle = person.avatarUrl ? { backgroundImage: `url("${person.avatarUrl}")` } : undefined; return <button key={person.userId} type="button" role="option" aria-selected={active} onClick={() => { onChange({ assignee: displayLabel, assigneeId: person.userId }); onOpenChange(false); }} className="mt-2 flex w-full items-center gap-2 rounded-md px-1 py-1.5 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800"><span aria-hidden="true" style={avatarStyle} className={`grid h-6 w-6 place-items-center rounded-full bg-violet-500 bg-cover bg-center text-[10px] font-bold text-white ${person.avatarUrl ? '' : ''}`}>{person.avatarUrl ? '' : person.initials}</span><span className="min-w-0 flex-1"><span className="block truncate">{displayLabel}</span>{!isCurrentUser && person.email && <span className="block truncate text-xs text-slate-500">{person.email}</span>}</span>{active && <Check aria-hidden="true" size={14} />}</button>; })}{!filteredPeople.length && !inviteEmail && <p className="px-1 py-2 text-sm text-slate-500">No people found.</p>}{inviteEmail && <button type="button" role="option" aria-selected={value === inviteEmail} onClick={() => { onChange({ assignee: inviteEmail, assigneeId: null }); onOpenChange(false); }} className="mt-2 flex w-full items-center gap-2 rounded-md px-1 py-1.5 text-left text-sm hover:bg-slate-100 dark:hover:bg-slate-800"><UserRound aria-hidden="true" size={17} className="text-indigo-500" /><span>Invite {inviteEmail}</span></button>}<button type="button" className="mt-2 flex items-center gap-2 text-sm font-medium text-slate-700 hover:text-indigo-600 dark:text-slate-200"><Plus size={15} />Invite people via email</button></div></div>}</div>;
}
