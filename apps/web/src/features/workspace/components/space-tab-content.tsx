...
function SpaceGantt({ projects, onOpenTask }: { projects: Project[]; onOpenTask: (projectId: string, taskId: string) => void }) {
  ...
  const taskSchedule = new Map(projects.flatMap((project) => project.tasks).map((task, index) => [task.id, schedule[index % schedule.length]]));
