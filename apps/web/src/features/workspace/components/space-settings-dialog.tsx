...
export function SpaceSettingsDialog({ open, name, isPrivate, icon, tone, pending, onOpenChange, onNameChange, onPrivateChange, onIconChange, onToneChange, onSubmit }: { open: boolean; name: string; isPrivate: boolean; icon: string; tone: string; pending: boolean; onOpenChange: (open: boolean) => void; onNameChange: (value: string) => void; onPrivateChange: (value: boolean) => void; onIconChange: (value: string) => void; onToneChange: (value: string) => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void }) {
  return <Dialog open={open} onOpenChange={onOpenChange} ariaLabel="Space settings">
    <form onSubmit={onSubmit} className="space-y-5"><div><DialogTitle>Space settings</DialogTitle><p className="mt-1 text-sm text-slate-500">Only the Space owner can change these settings.</p></div>
      ...
      <fieldset><legend className="text-sm font-semibold">Access</legend><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" aria-pressed={!isPrivate} onClick={() => onPrivateChange(false)} className={`rounded-lg border px-3 py-2 text-sm ${!isPrivate ? 'border-indigo-500 bg-indigo-50 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-200' : 'border-slate-200 dark:border-slate-700'}`}>Public</button><button type="button" aria-pressed={isPrivate} onClick={() => onPrivateChange(true)} className={`rounded-lg border px-3 py-2 text-sm ${isPrivate ? 'border-indigo-500 bg-indigo-50 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-200' : 'border-slate-200 dark:border-slate-700'}`}>Private</button></div></fieldset>
