...
import { Bookmark, CalendarDays, Columns3, FileText, Folder, GanttChartSquare, LayoutDashboard, LayoutList, Link2, ListChecks, LockKeyhole, Plus, RefreshCw, Settings2, Table2, Users, X } from 'lucide-react';
...
export function SpaceOverview() {
  ...
  const canShareSelectedSpace = selectedLocalSpace?.role === 'OWNER';
  ...
  const submitInviteMember = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!email || !selectedLocalSpace) return;
    if (!navigationQuery.usesApi) {
      toast.info('Member invitations require an API-backed Space.');
      return;
    }
    try {
      await inviteMemberMutation.mutateAsync({ workspaceId: selectedLocalSpace.id, email });
      setInviteEmail('');
      toast.success('Member invited to this Space.');
    } catch { toast.error('Unable to invite this member.'); }
  };
