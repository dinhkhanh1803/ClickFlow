...
export function SpaceOverview() {
  ...
  const createLocalListTask = (input: { title: string; status: LocalTaskStatus }) => {
    if (!selectedLocalSpace || !selectedList) return;
    const nextSpaces = localSpaces.map((localSpace) => localSpace.id === selectedLocalSpace.id ? {
      ...localSpace,
      items: localSpace.items.map((item) => item.id === selectedList.id ? {
        ...item,
        tasks: [...(item.tasks ?? []), ({ id: localId('task'), title: input.title, status: input.status, priority: 'Normal', assignee: '', startDate: '', dueDate: '', timeEstimate: '', trackingStartedAt: null, trackedSeconds: 0, tags: [], description: '', comments: [], attachments: [], createdAt: new Date().toISOString() } satisfies LocalListTask)],
      } : item),
    } : localSpace);
    setLocalSpaces(nextSpaces);
    saveLocalSpaces(nextSpaces);
