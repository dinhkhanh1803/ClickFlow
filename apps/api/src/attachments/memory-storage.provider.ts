...
export class MemoryStorageProvider implements StorageProvider {
  ...
  createSignedUpload(spec: UploadIntentSpec) { return Promise.resolve({ uploadUrl: `memory://upload/${encodeURIComponent(spec.storageKey)}`, uploadMethod: 'PUT' as const, uploadHeaders: { 'Content-Type': spec.mimeType }, expiresAt: new Date(Date.now() + spec.expiresInSeconds * 1000) }); }
